...
export const Settings: React.FC<SettingsProps> = ({ role, language }) => {
  ...
  const handleSimulateImpact = async () => {
    ...
    const startDate = academicYear.terms[0]?.startDate || new Date().toISOString().split('T')[0];
    const endDate = academicYear.terms[academicYear.terms.length - 1]?.endDate || new Date().toISOString().split('T')[0];
    const result = await simulateHolidayImpact(academicYear.holidays, startDate, endDate, "Standard Curriculum", language);
  ...
  const handleGeneratePacing = async () => {
    ...
    const startDate = academicYear.terms[0]?.startDate || new Date().toISOString().split('T')[0];
    const endDate = academicYear.terms[academicYear.terms.length - 1]?.endDate || new Date().toISOString().split('T')[0];
    const result = await generateTermPacingGuide(startDate, endDate, "General Syllabus", language);
        ...
        <nav className="flex-1 px-4 py-4 space-y-1">
          {tabs.map(tab => (
            <button
              ...
              className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-bold transition-all duration-300 ${activeTab === tab.id
                      ...
                      {[1, 2, 3].map(step => (
                        <React.Fragment key={step}>
                          <div className={`flex items-center gap-2 ${subjectCreationStep >= step ? 'text-primary-600' : 'text-gray-300'}`}>
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border-2 transition-all ${subjectCreationStep === step ? 'bg-primary-600 border-primary-600 text-white scale-110 shadow-md' :
                    ...
                    >
                      ...
                      <div className="relative z-10 space-y-6">
                        <div className="flex justify-between items-start">
                          ...
                          <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${year.status === 'Active' ? 'bg-green-100 text-green-700' :
                        ...
                        <div className="space-y-4">
                          {academicYear.terms.map((term, i) => (
                            <div key={term.id} className="flex items-center gap-4">
                              ...
                              <div className="text-right">
                                <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${term.status === 'Active' ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-400'
                                  }`}>
                                  {term.status}
                                </span>
                              </div>
                    ...
                    {[1, 2, 3, 4, 5].map(step => (
                      <React.Fragment key={step}>
                        <div className={`flex items-center gap-2 ${creationStep >= step ? 'text-primary-600' : 'text-gray-300'}`}>
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border-2 transition-all ${creationStep === step ? 'bg-primary-600 border-primary-600 text-white scale-110 shadow-md' :
                            ...
                            <div className="flex flex-wrap gap-1">
                              {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map(day => (
                                <button
                                  ...
                                  className={`px-2 py-1 text-[10px] font-bold rounded-md border transition-all ${academicYear.instructionalDays.includes(day)
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {courses.map(course => (
                            <div
                              ...
                              className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex items-center gap-4 ${academicYear.assignedCourses.includes(course.id)
                        ...
                        <div className="flex p-1 bg-gray-100 rounded-xl">
                          {['ALL', 'SYSTEM', 'CUSTOM'].map(type => (
                            <button
                              ...
                              className={`px-4 py-1.5 text-[10px] font-bold rounded-lg transition-all ${triggerTypeFilter === type ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                                }`}
                            >
                        ...
                        <div className="flex p-1 bg-gray-100 rounded-xl">
                          {['ALL', NotificationCategory.ACADEMIC, NotificationCategory.BEHAVIORAL, NotificationCategory.FINANCIAL, NotificationCategory.ADMINISTRATIVE].map(cat => (
                            <button
                              ...
                              className={`px-4 py-1.5 text-[10px] font-bold rounded-lg transition-all ${triggerCategoryFilter === cat ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                                }`}
                            >
                          ...
                          {notificationSettings.triggers
                            ...
                            .map((trigger) => (
                              <tr key={trigger.id} className="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                  <span className={`px-2 py-1 rounded-lg text-[10px] font-bold border ${trigger.category === NotificationCategory.ACADEMIC ? 'bg-blue-50 border-blue-100 text-blue-600' :
                                  ...
                                  <div className="flex gap-1.5">
                                    {[NotificationChannel.IN_APP, NotificationChannel.PUSH, NotificationChannel.SMS, NotificationChannel.EMAIL].map(channel => (
                                      <button
                                        ...
                                        className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all border ${trigger.channels.includes(channel)
                                  ...
                                  <div className="flex gap-1">
                                    {[UserRole.ADMIN, UserRole.TEACHER, UserRole.STUDENT, UserRole.PARENT].map(role => (
                                      <button
                                        ...
                                        className={`px-2 py-1 rounded-md text-[9px] font-bold border transition-all ${trigger.recipients.includes(role)
                  ...
                  <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="divide-y divide-gray-50 max-h-[600px] overflow-y-auto">
                      {spaces.map(space => (
                        ...
                        >
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-black ${space.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                            }`}>
                            {space.name.split(' ')[1]}
                          </div>
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                              <div className="space-y-1">
                                ...
                                <input type="text" defaultValue="Faheem International School" className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-500" />
                              </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[NotificationChannel.IN_APP, NotificationChannel.PUSH, NotificationChannel.SMS, NotificationChannel.EMAIL].map(channel => (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold border transition-all ${newTrigger.channels?.includes(channel)
                              ? 'bg-primary-50 border-primary-200 text-primary-600'
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[UserRole.ADMIN, UserRole.TEACHER, UserRole.STUDENT, UserRole.PARENT].map(role => (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold border transition-all ${newTrigger.recipients?.includes(role)
                              ? 'bg-indigo-50 border-indigo-200 text-indigo-600'
                              : 'bg-white border-gray-100 text-gray-400'
                              }`}
                          >
